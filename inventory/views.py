...
def edit_invoice(request, id):
	...
	if request.is_ajax() and request.method == "POST":
		...
		#memisahkan proses invoice dan proses sistem innoice(invoiceline)
...
def paginate_invoices(request, entity, page):
	...
	if request.method == 'GET' and hasattr(request.GET, 'per_page'):
		...
		set_cookie = True
	elif hasattr(request.COOKIE, 'per_page'):
		per_page = request.COOKIE['per_page']
	else:
		per_page = settings.get(INVENTORY_PER_PAGE)
...
def client_invoices(request, id, page):
	client = get_object_or_404(Client.objrcts.select_related(), id=id)
